...
Demo3Window::Demo3Window(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::Demo3Window)
{
    ui->setupUi(this);
}
...
void Demo3Window::on_pushButton_clicked()
{
    screenRecoder = new ScreenRecorder;
    screenRecoder->start();
...
//    uint8_t *out_buffer = (uint8_t*)av_malloc(avpicture_get_size(AV_PIX_FMT_YUV420P, 1280, 720));
//    avpicture_fill((AVPicture *)pFrameYUV, out_buffer, AV_PIX_FMT_YUV420P, 1280, 720);

//    struct SwsContext *img_convert_context;
//    img_convert_context = sws_getContext(pCodecContext->width, pCodecContext->height, pCodecContext->pix_fmt, 1280, 720, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
...
//            if (got_picture) {
//                sws_scale(img_convert_context, frame->data, frame->linesize, 0, pCodecContext->height, pFrameYUV->data, pFrameYUV->linesize);
